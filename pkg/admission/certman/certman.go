...
//
// update by niusmallnan: move logger to logrus
package certman
...
func (cm *CertMan) Watch() error {
	...
	if err := cm.load(); err != nil {
		logrus.Infof("certman: can't load cert or key file: %v", err)
	}
	logrus.Info("certman: watching for cert and key change")
...
func (cm *CertMan) load() error {
	...
	if err == nil {
		...
		cm.keyPair = &keyPair
		cm.mu.Unlock()
		logrus.Info("certman: certificate and key loaded")
	}
	return err
}
...
loop:
	for {
		...
		case event := <-cm.watcher.Events:
			logrus.Infof("certman: watch event: %v", event)
			if err := cm.load(); err != nil {
				logrus.Infof("certman: can't load cert or key file: %v", err)
			}
		case err := <-cm.watcher.Errors:
			logrus.Infof("certman: error watching files: %v", err)
		}
	}
	logrus.Info("certman: stopped watching")
	cm.watcher.Close()
}
