...
func (h *Handler) syncMacvlanPodIP(s string, pod *corev1.Pod) (*corev1.Pod, error) {
	...
	if annotationIP == "auto" {
		...
		macvlanipType = "auto"
		if existMacvlanIP != nil && existMacvlanIP.Name != "" {
			logrus.Debugf("macvlan IP already exists: %v", utils.PrintObject(existMacvlanIP))
			// For statefulset pod.
			...
			logrus.Debugf("macvlanIP already exists, CIDR %q", existMacvlanIP.Spec.CIDR)
		} else {
...
func (h *Handler) syncService(pod *corev1.Pod) error {
	...
	err = retry.OnError(svcGetRetryBackoff, k8serrors.IsNotFound, func() error {
