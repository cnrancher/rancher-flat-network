...
type handler struct {
	subnetClient flcontroller.SubnetClient
...
func Register(
	...
	h := &handler{
		...
		subnetEnqueueAfter: wctx.FlatNetwork.Subnet().EnqueueAfter,
		subnetEnqueue:      wctx.FlatNetwork.Subnet().Enqueue,
	}

	wctx.FlatNetwork.Subnet().OnChange(ctx, handlerName, h.handleError(h.handleSubnet))
	wctx.FlatNetwork.Subnet().OnRemove(ctx, handlerRemoveName, h.handleSubnetRemove)
}

func (h *handler) handleError(
	onChange func(string, *flv1.Subnet) (*flv1.Subnet, error),
) func(string, *flv1.Subnet) (*flv1.Subnet, error) {
	return func(key string, subnet *flv1.Subnet) (*flv1.Subnet, error) {
		var err error
		var message string
...
func ip2UsedRanges(ips []*flv1.IP) []flv1.IPRange {
