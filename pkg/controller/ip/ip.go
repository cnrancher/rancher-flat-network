...
func Register(
	...
	h := &handler{
		...
		ipEnqueueAfter: wctx.FlatNetwork.IP().EnqueueAfter,
		ipEnqueue:      wctx.FlatNetwork.Subnet().Enqueue,

		podEnqueueAfter: wctx.Core.Pod().EnqueueAfter,
		podEnqueue:      wctx.Core.Pod().Enqueue,
	}

	wctx.FlatNetwork.IP().OnChange(ctx, handlerName, h.handleError(h.handleIP))
	wctx.FlatNetwork.IP().OnRemove(ctx, handlerRemoveName, h.handleIPRemove)
}

func (h *handler) handleError(
	onChange func(string, *flv1.IP) (*flv1.IP, error),
) func(string, *flv1.IP) (*flv1.IP, error) {
	return func(key string, ip *flv1.IP) (*flv1.IP, error) {
		var message string
		var err error
...
func (h *handler) handleIP(s string, ip *flv1.IP) (*flv1.IP, error) {
...
func (h *handler) onIPCreate(ip *flv1.IP) (*flv1.IP, error) {
