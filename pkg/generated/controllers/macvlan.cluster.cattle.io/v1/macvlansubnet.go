...
// MacvlanSubnetStatusHandler is executed for every added or modified MacvlanSubnet. Should return the new status to be updated
type MacvlanSubnetStatusHandler func(obj *v1.MacvlanSubnet, status v1.MacvlanSubnetStatus) (v1.MacvlanSubnetStatus, error)

// MacvlanSubnetGeneratingHandler is the top-level handler that is executed for every MacvlanSubnet event. It extends MacvlanSubnetStatusHandler by a returning a slice of child objects to be passed to apply.Apply
...
// RegisterMacvlanSubnetStatusHandler configures a MacvlanSubnetController to execute a MacvlanSubnetStatusHandler for every events observed.
...
// RegisterMacvlanSubnetGeneratingHandler configures a MacvlanSubnetController to execute a MacvlanSubnetGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured MacvlanSubnetGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *macvlanSubnetGeneratingHandler) isNewResourceVersion(obj *v1.MacvlanSubnet) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *macvlanSubnetGeneratingHandler) storeResourceVersion(obj *v1.MacvlanSubnet) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
