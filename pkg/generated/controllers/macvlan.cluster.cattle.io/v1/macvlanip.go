...
// MacvlanIPStatusHandler is executed for every added or modified MacvlanIP. Should return the new status to be updated
type MacvlanIPStatusHandler func(obj *v1.MacvlanIP, status v1.MacvlanIPStatus) (v1.MacvlanIPStatus, error)

// MacvlanIPGeneratingHandler is the top-level handler that is executed for every MacvlanIP event. It extends MacvlanIPStatusHandler by a returning a slice of child objects to be passed to apply.Apply
type MacvlanIPGeneratingHandler func(obj *v1.MacvlanIP, status v1.MacvlanIPStatus) ([]runtime.Object, v1.MacvlanIPStatus, error)

// RegisterMacvlanIPStatusHandler configures a MacvlanIPController to execute a MacvlanIPStatusHandler for every events observed.
...
// RegisterMacvlanIPGeneratingHandler configures a MacvlanIPController to execute a MacvlanIPGeneratingHandler for every events observed, passing the returned objects to the provided apply.Apply.
// If a non-empty condition is provided, it will be updated in the status conditions for every handler execution
...
// Handle executes the configured MacvlanIPGeneratingHandler and pass the resulting objects to apply.Apply, finally returning the new status of the resource
...
func (a *macvlanIPGeneratingHandler) isNewResourceVersion(obj *v1.MacvlanIP) bool {
	if !a.opts.UniqueApplyForResourceVersion {
		return true
	}
...
// Only used if UniqueApplyForResourceVersion is set in generic.GeneratingHandlerOptions
func (a *macvlanIPGeneratingHandler) storeResourceVersion(obj *v1.MacvlanIP) {
	if !a.opts.UniqueApplyForResourceVersion {
		return
	}
